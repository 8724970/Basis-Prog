...
/**
 * Alert can be used to display an alert massage.
 * 
...
public class Alert extends Actor
{
    ...
    static private Color bgColor       = new Color (246,141,130),
    ...
    public static boolean alertActive( World world ) {
        return world.getObjects( Alert.class ).size() > 0;
    }
    
    public Alert () {
        this( "Dit is een heel lange tekst" );
    }
    public Alert( String message_text ) {
        GreenfootImage board_image = new GreenfootImage( MB_WIDTH, MB_HEIGHT );
        drawBox( 0, 0, MB_WIDTH, MB_HEIGHT, BORDER_WIDTH, board_image, bgColor);
        drawPicture( "frown.png", MB_WIDTH, 10, board_image);
        drawText( message_text, 0, TXT_Y, MB_WIDTH, board_image, new Font("Arial", Font.BOLD, 16), textColor );
        drawBox( (MB_WIDTH - OK_WIDTH)/2, OK_Y, OK_WIDTH, OK_HEIGHT, BORDER_WIDTH, board_image, buttonColor );
        drawText( "OK", (MB_WIDTH - OK_WIDTH)/2, OK_Y+OK_HEIGHT-10, OK_WIDTH, board_image, new Font("Arial", Font.BOLD, 16), textColor );
    ...
    private void drawText( String text, int x, int y, int maxw, GreenfootImage image, Font font, Color color ) {
        image.setFont( font );
        FontMetrics fm = image.getAwtImage().getGraphics().getFontMetrics( font );
